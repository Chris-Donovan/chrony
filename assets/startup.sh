...
## dynamically populate chrony config file.
{
  ...
  echo "# time server pools provided by NTP_POOLS environment variable."
} > ${CHRONY_CONF_FILE}


# NTP_SERVERS and NTP_POOLS environment variables are not present, so populate with default servers
if [ -z "${NTP_SERVERS}" ] && [ -z "${NTP_POOLS}" ]; then
...
for N in $NTP_POOLS; do
  ...
  if [[ "${ENABLE_NTS:-false}" = true ]]; then
    echo "pool "${N_CLEANED}" iburst nts" >> ${CHRONY_CONF_FILE}
  else
